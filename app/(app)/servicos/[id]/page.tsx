...
export default async function ServicoPage({ params }: { params: { id: string } }) {
  ...
  const canFinalize = !isReadonly && pendentes === 0 && problemasSemObs === 0
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      <div className="flex items-start justify-between mb-6 gap-3 flex-wrap">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-bold text-vobi-dark truncate">{servico.cliente_nome}</h1>
              {isReadonly ? (
        ...
        {!isReadonly && (
          <FinalizeDialog
            ...
            pendentes={pendentes}
            problemasSemObs={problemasSemObs}
          />
        )}
        {isReadonly && (
      ...
      {/* Diagnóstico IA (≥50% ou já gerado) */}
      <DiagnoseButton
        servicoId={servico.id}
        progressPct={pct}
